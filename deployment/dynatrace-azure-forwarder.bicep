...
@description('Dynatrace destination. Could be live service or ActiveGate URL.')
param targetUrl string

@description('Dynatrace Paas Token if using ActivateGate. Optional if not exists.')
@secure()
...
param functionName string = '${forwarderName}-function'
@description('Name of the EventHubNamespace to deploy.')
param eventhubNamespaceName string = '${forwarderName}-eventhub'
@description('Name of the supporting storage account to create.')
param storageAccountNameParam string = '${forwarderName}storage'
@description('Name of the User Assigned Managed Identity to use for the deploy.')
param userAssignedIdentityName string = '${forwarderName}-id'

var storageAccountName = storageAccountNameParam == '' ? '${forwarderNameShort}sa${randomIdToMakeStorageAccountGloballyUnique}' : storageAccountNameParam
...
resource userAssignedIdentityName_roleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(resourceGroup().id, userAssignedIdentity.id, 'de139f84-1756-47ae-9be6-808fbbe84772') //Website Contributor
  properties: {
...
resource functionCodeDeploymentScript 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  ...
  properties: {
    azCliVersion: '2.42.0'
    scriptContent: '''
      echo "- downloading function code zip [$FUNCTION_PACKAGE_URL$FUNCTION_PACKAGE_NAME]"
      wget -q $FUNCTION_PACKAGE_URL$FUNCTION_PACKAGE_NAME -O $FUNCTION_PACKAGE_NAME
      az webapp deploy -n $FUNCTIONAPP_NAME -g $FUNCTIONAPP_RESOURCEGROUP --src-path $FUNCTION_PACKAGE_NAME --type zip --async true
    '''
    retentionInterval: 'P1D'
    arguments: ''
    timeout: 'PT30M'
    environmentVariables: [
      {
        ...
        value: 'https://github.com/dynatrace-oss/dynatrace-azure-log-forwarder/releases/latest/download/'
      }
      {
      ...
      }
      {
        ...
        value: function.properties.resourceGroup
      }
    ]
  }
}
